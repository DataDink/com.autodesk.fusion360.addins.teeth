...
def run(context):
  try:
    ...
    if command: return
    command = ui.commandDefinitions.addButtonDefinition(id, name, description, './resources')
    command.commandCreated.add(outward)
    panel.controls.addCommand(command)
    inward = Inward()
    factories.append(inward)
    invertedId = id + '-inverted'
    command = ui.commandDefinitions.addButtonDefinition(invertedId, name + ' (Inverted)', description, './resources')
